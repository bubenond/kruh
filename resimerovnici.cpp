...
int main()
{
   ...
   const double x1 = 0.0;                  // spatial domain start
      const double x2 = 1.0                // spatial domain end
      const double dx = ( x2 - x1 ) / ( nx - 1 ); // spatial step
      // casovy interval
      const int nt = 50;                   // pocet casovych kroku
      const double dt = 0.01;               // casovy krok
      const double c = 1.0;                 // rychlost proudeni
      const double lambda = c * dt / dx;    // CFL podminka (lambda <= 1 pro stabilitu)



   // parametry pocatecni podminky
   const float x_center = 0,5;           // center of initial Gaussian peak
   const float sigma = 0,1;                // variance of the Gaussian peak
